...
test_that("label counting per neighborhood works", {
  ...
  testthat::expect_warning(
    count_labels_per_neighborhood(nei, labels, return = "matrix")
  )

  nei$neighborhood <- map(nei$neighborhood, \(x) paste0("cell_", x))
  labels <- rep(letters[1:2], times = 5)
  tidy_res3 <- count_labels_per_neighborhood(nei, labels, cell_names = paste0("cell_", 1:10), return = "tidy")
  expect_equal(tidy_res3, tidy_res1)

  nei <- tibble(name ="gene_1", neighborhood = list(c(10)))
  labels <- c('a','b','a')
  expect_error(
    count_labels_per_neighborhood(nei, labels, return = "tidy")
  )
  ...
  count_labels_per_neighborhood(nei, vars(a, test = "123"), fit = sce)
  mat_res2 <- count_labels_per_neighborhood(nei, vars(a), fit = sce, return = "matrix")
  expect_warning(
